"""Day-16 benchmark: same six-scene workload through the optimized renderer.

Same saved six-scene plan as Days 11-13 (identical plan bytes), fresh real
System.Speech audio, fresh scene media and a fresh MP4. No mocks, no reused
media, no shortened workload. Only the Day-16 opaque line fast path
is under measurement; no other production optimization.
"""
...
from scripts.day16_instrumentation import FrameProfiler, delta
...
DAY16_BASELINE = {
    'line_drawing_seconds': 58.592556800006605,
    'frame_generation_seconds': 172.31925310001816,
    'render_seconds': 205.38504880000028,
    'rendered_fps': 8.759157545843705,
    'real_time_factor': 3.4249868981379317,
}
...
def main():
    ...
    parser.add_argument('--output-dir', type=Path, default=PROJECT_ROOT / 'output/day17_profiling')
    ...
    assert '_blit_span' in line_source, 'Day-16 opaque span fast path not active'
...
def finish(root, job_dir, source, plan_bytes, protected, hashes, artifact, raw, scenes, stages, wall):
    ...
    comparison = {}
    for key, baseline in DAY16_BASELINE.items():
        value = measured[key]
        entry = {'day16': baseline, 'day17': value}
    ...
    report = {
        ...
        'day17_comparison_vs_day16': comparison,
        'notes': [
            'Same saved six-scene input (plan bytes identical to Day-11/Day-12/Day-13); no upstream agents or mocks executed; all scene media rendered afresh.',
            ...
            'Profiling-only Day 17: no production code changed since Day 16. Only measurement is under consideration.',
            'No progress artifact synthesized. Verifier unchanged.',
        ],
    }
    save(root / 'day17_render_profile.json', report)
    print(json.dumps({k: report[k] for k in ('render_status', 'render_seconds', 'frame_generation_seconds',
                                             ...
                                             'day17_comparison_vs_day16')}, indent=2), flush=True)
