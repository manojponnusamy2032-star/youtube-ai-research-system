"""Day-15 fresh profile: unchanged six-scene workload through real production rendering.
...
media, no shortened workload. Profiling only; no production optimization.
"""
...
from scripts.day15_instrumentation import FrameProfiler, delta
...
DAY13_TRUSTED_BASELINE = {
    'line_drawing_seconds': 42.240837499528425,
    'frame_generation_seconds': 129.65139140004248,
    'render_seconds': 161.3098800000007,
    'rendered_fps': 11.152447698801785,
    'real_time_factor': 2.6899924252918828,
}
...
def main():
    ...
    parser.add_argument('--output-dir', type=Path, default=PROJECT_ROOT / 'output/day15_profiling')
...
def finish(root, job_dir, source, plan_bytes, protected, hashes, artifact, raw, scenes, stages, wall):
    ...
    comparison = {}
    for key, baseline in DAY13_TRUSTED_BASELINE.items():
        value = measured[key]
        entry = {'day13': baseline, 'day15': value}
    ...
    report = {
        ...
        'day15_comparison_vs_day13': comparison,
        'notes': [
            ...
            'No production change under measurement (profiling-only day); renderer expected unchanged from Day-13.',
            'No progress artifact synthesized. Verifier unchanged.',
        ],
    }
    save(root / 'day15_render_profile.json', report)
    print(json.dumps({k: report[k] for k in ('render_status', 'render_seconds', 'frame_generation_seconds',
                                             ...
                                             'day15_comparison_vs_day13')}, indent=2), flush=True)
