"""Run the unchanged six-scene workload through real production rendering."""
...
def main():
    ...
    parser.add_argument('--output-dir', type=Path, default=PROJECT_ROOT / 'output/day12_profiling')
    root = parser.parse_args().output_dir.resolve()
    job_dir = root / 'jobs/job_001'
    job_dir.mkdir(parents=True, exist_ok=False)
    work = root / 'work'
    work.mkdir(exist_ok=False)
    source = PROJECT_ROOT / 'output/day11_optimization/benchmark/jobs/job_001/visual_plan.json'
...
def finish(root, job_dir, source, plan_bytes, protected, hashes, artifact, raw, scenes, stages, wall):
    ...
    report = {
        ...
        'notes': [
            'Same saved six-scene Day-11 input; no upstream agents or mocks executed; all scene media rendered afresh.',
            'Adapter wall excludes setup/probing/tests and upstream planning; Day-11 wall included upstream deterministic planning.',
            'Fresh real System.Speech cache; Day-11 may have used cached audio. Do not attribute audio variation to raster performance.',
            'Stage, owner and method inclusive timings overlap. Never add them to the exclusive breakdown.',
            'Background exclusive excludes inline allocation/fill/floor/sun; characters exclusive excludes primary inline code. See owner timings and report.',
            'Alpha blending cannot be isolated cheaply by per-call wrappers; remains inside rectangle/line/circle times.',
            'Geometry is partial helper self time inside frames. Pre-frame motion/pose is in method totals but excluded from frame breakdown.',
            'No polygon rasterizer or ellipse-specific/shadow pass on this production path; circles are the ellipses bucket.',
            'FFmpeg runs concurrently; independent encode time unknown.',
            'No progress artifact synthesized. Verifier unchanged.',
        ],
    }
    save(root / 'day12_render_profile.json', report)
    print(json.dumps({k: report[k] for k in ('render_status', 'render_seconds', 'frame_generation_seconds', 'breakdown', 'bottleneck', 'verifier_exit_code')}, indent=2), flush=True)
